...
spi_df = spi_simulation.calculate_dataframe(VARIABLES_TO_ADJUST + ["age"], period=2019)
...
for variable in VARIABLES_TO_ADJUST:
    ...
    percentile_adjusted_pensioner_split_frs_person_df[variable][is_frs_pensioner] = match_percentiles(
        frs_df[variable][is_frs_pensioner], spi_df[variable][is_spi_pensioner],
        percentile_threshold=0.97,
        num_groups=12,
    )
    percentile_adjusted_pensioner_split_frs_person_df[variable][~is_frs_pensioner] = match_percentiles(
...
percentile_adjusted_pensioner_split_dataset = create_frs_dataset(percentile_adjusted_pensioner_split_frs_person_df)
...
percentile_adjusted_pensioner_split_loss = loss(household_weights, percentile_adjusted_pensioner_split_dataset)
...
print(f"Percentile adjusted pensioner split loss: {percentile_adjusted_pensioner_split_loss}")
